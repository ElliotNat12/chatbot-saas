  ...
  function init(cfg) {
    ...
    function addToCart(variantId, fallbackUrl) {
      ...
      if (cartItems.length === 1) {
        ...
        btnDebardeur.textContent = currentLang === 'en' ? 'Add a tank top' : 'Ajouter un débardeur';
        btnDebardeur.addEventListener('click', () => { wrap.remove(); sendMsg(currentLang === 'en' ? 'nursing tank top' : 'débardeur'); }, { once: true });
        const btnPads = document.createElement('button');
        btnPads.className = 'cb-color-btn';
        btnPads.textContent = currentLang === 'en' ? 'Add nursing pads' : 'Ajouter des pads';
        btnPads.addEventListener('click', () => { wrap.remove(); sendMsg(currentLang === 'en' ? 'nursing pads' : 'pads'); }, { once: true });
        const btnCart = document.createElement('button');
        btnCart.className = 'cb-color-btn';
        btnCart.textContent = currentLang === 'en' ? 'View my cart' : 'Voir mon panier';
